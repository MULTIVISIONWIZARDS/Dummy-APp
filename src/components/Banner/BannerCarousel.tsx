...
export default function BannerCarousel({
  ...
  topDot = null,
  tickDuration = 20000,
  tickerOverlay = false, // true = overlay ticker inside image
}) {
  ...
  // --- Parse text with {red} tags ---
  const parts = [];
  const regex = /\{green\}(.*?)\{\/green\}/g;
  let lastIndex = 0;
  let match;
  while ((match = regex.exec(text)) !== null) {
    if (match.index > lastIndex) {
    ...
    parts.push({ text: match[1], color: "green" });
    lastIndex = regex.lastIndex;
  }
  if (lastIndex < text.length) {
    parts.push({ text: text.slice(lastIndex), color: "white" });
      ...
      >
        ...
        >
          {parts.map((part, index) => (
            <Text
              key={index}
              style={part.color === "green" ? styles.red : styles.white}
            >
              {part.text}
            </Text>
          ))}
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [activeIndex, data.length, loading, startProgress]);
  ...
  const renderItem = useCallback(
    ...
    [bannerHeight, borderRadius, onPress, showText, ticker, tickerOverlay, width]
  );

  // --- Optimization for FlatList ---
